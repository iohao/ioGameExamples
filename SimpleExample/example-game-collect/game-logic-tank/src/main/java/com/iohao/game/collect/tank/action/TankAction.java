...
public class TankAction {
    static TankRoomService roomService = new TankRoomService();
    /** 开发阶段，只用一个房间 */
...
//        }
    ...
    public TankEnterRoom enterRoom(FlowContext flowContext, TankEnterRoom enterRoom) {
        ...
        // 房间
        TankRoomEntity room = (TankRoomEntity) roomService.optionalRoom(roomId).orElseGet(() -> {
            // 房间不存在，创建一个房间
            RoomCreateContext roomCreateContext = RoomCreateContext.of(flowContext);
            TankRoomEntity newRoom = roomService.createRoom(roomCreateContext);
            // TODO: 2022/1/14 开发阶段，只用一个房间
            newRoom.setRoomId(tempRoomId);
        ...
        roomService.enterRoom(gameFlowContext);
